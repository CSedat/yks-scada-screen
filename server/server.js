...
const { Ceviz, Araurun, Toz, Findik } = require("./dbSchema");
...
app.post("/saveData", async function (req, res) {
  ...
  try {
    ...
    if (req.body.urun === "ceviz") {
      const lasId = await Ceviz.find().sort({ id: -1 }).limit(1);
      Ceviz.create(req.body);
    } else if (req.body.urun === "findik") {
      Findik.create(req.body);
    } else if (req.body.urun === "toz") {
      Toz.create(req.body);
    } else if (req.body.urun === "araurun") {
      Araurun.create(req.body);
    }
  } catch (error) {
    console.log(error)
    throw error;
